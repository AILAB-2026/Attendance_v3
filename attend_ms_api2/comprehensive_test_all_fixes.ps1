...
Write-Host "ðŸ§ª COMPREHENSIVE TEST - All Leave and Clock Out Fixes" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan

try {
    # Login first
    Write-Host "`n1. ðŸ” Testing Login..." -ForegroundColor Yellow
    $loginBody = @{
        companyCode = "1"
        employeeNo = "B1-E079"
        password = "Test@123"
    ...
    if ($loginResponse.success) {
        ...
        try {
            ...
            if ($leaveResponse -and $leaveResponse.Count -gt 0) {
                ...
                Write-Host "   ðŸ“Š Status breakdown: Approved: $approvedCount, Pending: $pendingCount, Rejected: $rejectedCount" -ForegroundColor Cyan
            } else {
                Write-Host "   âŒ No leave requests found" -ForegroundColor Red
            }
        } catch {
            Write-Host "   âŒ Leave requests error: $($_.Exception.Message)" -ForegroundColor Red
        }
        ...
        try {
            ...
            Write-Host "   âŒ Leave balance error: $($_.Exception.Message)" -ForegroundColor Red
        }
        ...
        try {
            ...
            if ($todayResponse.entries -and $todayResponse.entries.Count -gt 0) {
                Write-Host "   ðŸ“Š Found $($todayResponse.entries.Count) attendance entries for today:" -ForegroundColor Cyan
                ...
                Write-Host "   âš ï¸  No attendance entries for today (employee needs to clock in first)" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "   âŒ Today's attendance error: $($_.Exception.Message)" -ForegroundColor Red
        }
        ...
        try {
            ...
            if ($historyResponse -and $historyResponse.Count -gt 0) {
                Write-Host "   ðŸ“Š Found $($historyResponse.Count) history entries:" -ForegroundColor Cyan
                ...
                if ($todayEntries) {
                    Write-Host "   âœ… Today's date (Nov 12) found in history" -ForegroundColor Green
                } else {
                    Write-Host "   âš ï¸  Today's date (Nov 12) not found in history - employee may need to clock in today" -ForegroundColor Yellow
                }
            } else {
                Write-Host "   âŒ No history entries found" -ForegroundColor Red
            }
        } catch {
            Write-Host "   âŒ Attendance history error: $($_.Exception.Message)" -ForegroundColor Red
        }
        
        Write-Host "`nðŸŽ‰ SUMMARY OF FIXES:" -ForegroundColor Green
        Write-Host "===================" -ForegroundColor Green
        Write-Host "âœ… Leave Page: Fixed API endpoint to show applied leaves for B1-E079" -ForegroundColor Green
        Write-Host "âœ… Leave Status: Fixed mapping (validateâ†’approved, draftâ†’pending, refuseâ†’rejected)" -ForegroundColor Green
        Write-Host "âœ… Leave Filtering: Status mapping now supports All/Pending/Approved/Rejected filters" -ForegroundColor Green
        Write-Host "âœ… Clock Out: Fixed employee lookup query (x_Emp_No column)" -ForegroundColor Green
        Write-Host "âœ… History Page: Fixed timezone handling to show current date (Nov 12)" -ForegroundColor Green
        Write-Host "âœ… Database: All queries now use Singapore timezone for accurate date handling" -ForegroundColor Green
