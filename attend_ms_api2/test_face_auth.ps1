...
Write-Host "=== Face Authentication Test (Clock In/Out) ===" -ForegroundColor Cyan
...
Write-Host "Authenticating face..." -ForegroundColor Yellow
...
try {
    ...
    $response = Invoke-RestMethod -Uri "http://192.168.1.4:7012/facial-auth/authenticate" `
        -Method Post `
        -ContentType "multipart/form-data; boundary=$boundary" `
        -Body $requestBytes
    
    Write-Host ""
    Write-Host "SUCCESS! Face authenticated!" -ForegroundColor Green
    Write-Host ""
