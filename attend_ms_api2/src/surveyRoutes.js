...
router.get("/list", async (req, res) => {
    try {
        ...
        const surveyResult = await pool.query(
            `SELECT id, title, description, questions_layout, access_mode, active, create_date, survey_start_date, survey_end_date
       FROM survey_survey
       WHERE active = true
...
router.post("/:id/submit", async (req, res) => {
    try {
        ...
        const userInputResult = await pool.query(
            `INSERT INTO survey_user_input (survey_id, partner_id, state, create_date, write_date, start_datetime, end_datetime, access_token, nickname, employee_name, employee_no)
         VALUES ($1, $2, 'done', NOW(), NOW(), NOW(), NOW(), $3, $4, $5, $6)
         RETURNING id`,
        ...
        for (const ans of answers) {
            ...
            await pool.query(
             ...
             (user_input_id, survey_id, question_id, question_sequence, answer_type, value_char_box, value_text_box, value_numerical_box, suggested_answer_id, skipped, create_date, write_date)
