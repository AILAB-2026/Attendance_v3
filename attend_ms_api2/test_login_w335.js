...
console.log('ðŸ” Testing Login for B1-W335\n');
  ...
  try {
    ...
    `, [], async (err, res) => {
      ...
      if (res.rows.length === 0) {
        console.log('âŒ Employee B1-W335 not found');
        process.exit(1);
      }
      ...
      console.log(`  Active: ${employee.active ? 'âœ… ACTIVE' : 'âŒ INACTIVE'}`);
      ...
      if (loginData.success) {
        ...
        if (!loginData.data) {
          console.log('âŒ PROBLEM: No data object in response!');
          console.log('   Mobile app expects data object with user info');
        } else if (!loginData.data.sessionToken) {
          console.log('âŒ PROBLEM: No sessionToken in response!');
        ...
        console.log('âŒ Backend login failed');
      ...
      if (!employee.active) {
        console.log('âš ï¸  Employee B1-W335 is INACTIVE');
      ...
      if (loginData.success && loginData.data && loginData.data.sessionToken) {
        console.log('âœ… Login response is correct');
