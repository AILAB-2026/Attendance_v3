# Install NSSM service for AttendanceTestAPI (Node.js)
# Run this script as Administrator

param(
    [string]$ServiceName = "AttendanceTestAPI",
    [string]$NodePath = "C:\\Program Files\\nodejs\\node.exe",
    [string]$WorkingDir = "C:\\inetpub\\wwwroot\\attendance_test_api",
    [int]$Port = 7010
)
...
& $nssm set $ServiceName DisplayName "Attendance Test API Service" | Out-Null
& $nssm set $ServiceName Description "Node.js Attendance Test API (separate from production)" | Out-Null
& $nssm set $ServiceName Start SERVICE_AUTO_START | Out-Null

# Environment
$domain = "https://cx.brk.sg/attendance_test_api"
