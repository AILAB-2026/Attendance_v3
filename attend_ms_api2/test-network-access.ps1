...
try {
    ...
    Write-Host "   ❌ Network access: FAILED" -ForegroundColor Red
    Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Gray
}
...
try {
    $nodeResponse = Invoke-WebRequest -Uri "http://localhost:3001/health" -TimeoutSec 5 -ErrorAction Stop
    Write-Host "   ✅ Node.js direct: SUCCESS" -ForegroundColor Green
    Write-Host "   Status: $($nodeResponse.StatusCode)" -ForegroundColor White
} catch {
    Write-Host "   ❌ Node.js direct: FAILED" -ForegroundColor Red
    Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Gray
...
if (Get-Module WebAdministration) {
    ...
    if ($site) {
        Write-Host "   ✅ IIS Site Status: $($site.State)" -ForegroundColor Green
    ...
    Write-Host "   ⚠️  IIS Management not available" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "=== Test Summary ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "📱 For Mobile App Configuration:" -ForegroundColor Green
...
Write-Host "   Direct:  http://localhost:3001/health" -ForegroundColor White
Write-Host ""
