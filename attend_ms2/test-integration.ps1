...
try {
    $result = Invoke-RestMethod -Method GET -Uri "http://localhost:3000/health" -ErrorAction Stop
    Write-Host "✅ AIAttend_v2 backend is running" -ForegroundColor Green
    Write-Host "   Uptime: $($result.uptime) seconds" -ForegroundColor Gray
} catch {
    Write-Host "❌ AIAttend_v2 backend is NOT running" -ForegroundColor Red
...
try {
    $result = Invoke-RestMethod -Method GET -Uri "http://localhost:3001/health" -ErrorAction Stop
    Write-Host "✅ Face Recognition API is running (port 3001)" -ForegroundColor Green
    Write-Host "   Status: $($result.status)" -ForegroundColor Gray
} catch {
    Write-Host "❌ Face Recognition API is NOT running" -ForegroundColor Red
...
try {
    $result = Invoke-RestMethod -Method GET -Uri "http://localhost:3000/v1/face-health" -ErrorAction Stop
    if ($result.healthy) {
        Write-Host "✅ Face API integration is healthy" -ForegroundColor Green
    } else {
        Write-Host "⚠️  Face API integration reports unhealthy" -ForegroundColor Yellow
    ...
    Write-Host "❌ Face health endpoint failed" -ForegroundColor Red
...
Write-Host "   ℹ️  Run manually: psql -U postgres -d attendance_db -c '\dt user_face_mapping'" -ForegroundColor Gray
...
Write-Host ""
Write-Host "✅ AIAttend_v2 Backend:        http://localhost:3000" -ForegroundColor Green
Write-Host "✅ Face Recognition API:       http://localhost:3001" -ForegroundColor Green
Write-Host ""
