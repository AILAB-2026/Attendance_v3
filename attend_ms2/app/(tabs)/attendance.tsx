        ...
        <View style={styles.card}>
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryCard}>
              ...
              <Text style={[styles.summaryValue, isClockedIn() && !isClockedOut() ? styles.badgeSuccess : isClockedOut() ? styles.badgeNeutral : styles.badgeWarn]}>
                {isClockedIn() ? (isClockedOut() ? 'Clocked Out' : 'Clocked In') : 'Not Clocked In'}
              </Text>
            </View>
            {showHours && (
              <View style={styles.summaryCard}>
                <Text style={styles.summaryLabel}>Normal Hrs</Text>
                <Text style={styles.summaryValue}>{(todayAttendance?.normalHours ?? 0).toFixed(2)}</Text>
              </View>
            )}
            {showHours && (
              <View style={styles.summaryCard}>
                <Text style={styles.summaryLabel}>Overtime</Text>
                <Text style={[styles.summaryValue, (todayAttendance?.overtimeHours ?? 0) > 0 ? styles.badgeInfo : null]}>
                  {(todayAttendance?.overtimeHours ?? 0).toFixed(2)}
                </Text>
              </View>
            )}
          </View>
