...
import {
    ...
    ActivityIndicator, SafeAreaView, StatusBar, Dimensions, Alert
} from 'react-native';
...
export default function SurveyListScreen() {
    ...
    const renderItem = ({ item }: { item: any }) => {
        if (activeTab === 'history') {
            ...
            // Determine date to show (Submitted date for done, End/Create date for missed)
            ...
            return (
                <TouchableOpacity
                    style={[styles.historyCard, isMissed && { opacity: 0.9, borderColor: '#fee2e2' }]}
                    onPress={() => isMissed ? Alert.alert('Survey Closed', 'This survey is no longer available.') : handlePress(item.id)}
                    activeOpacity={0.9}
                >
                    <View style={styles.historyHeader}>
                        <Text style={styles.historyDate} numberOfLines={1}>{dateStr}</Text>
                        <View style={[styles.historyBadge, { backgroundColor: isMissed ? '#fef2f2' : '#dcfce7' }]}>
                            <Text style={[styles.historyBadgeText, { color: isMissed ? '#ef4444' : '#15803d' }]}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                        <Text style={{ fontSize: 12, color: isMissed ? '#ef4444' : '#94a3b8', fontStyle: 'italic' }}>
                        ...
                        {!isMissed ? (
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={{ color: '#ef4444', fontWeight: '700', fontSize: 13, marginRight: 4 }}>Closed</Text>
